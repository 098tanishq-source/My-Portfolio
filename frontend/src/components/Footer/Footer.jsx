...
const Footer = () => {
    ...
    return (
        <div style={{ background: '#080808', padding: '60px clamp(20px, 5vw, 60px) 40px', position: 'relative', zIndex: 2 }}>
            <style>{`
                .footer-link { color: rgba(240,236,228,0.4); font-size: 0.72rem; text-decoration: none; transition: color 0.2s; font-family: monospace; display: block; }
                .footer-link:hover { color: #f0ece4; }
                .footer-social { width: 34px; height: 34px; border-radius: 8px; border: 1px solid rgba(240,236,228,0.12); display: flex; align-items: center; justify-content: center; transition: all 0.2s; text-decoration: none; }
                .footer-social:hover { border-color: rgba(240,236,228,0.4); background: rgba(240,236,228,0.06); }
            ...
            {/* centered tilted card with tapes */}
            <div style={{ maxWidth: '900px', margin: '0 auto', position: 'relative' }}>
                ...
                <div style={{
                    ...
                    {/* small rip/torn edge effect top */}
                    <div style={{
                        position: 'absolute', top: '-2px', left: 0, right: 0,
                        height: '6px',
                        background: 'repeating-linear-gradient(90deg, #f0ece4 0px, #f0ece4 18px, transparent 18px, transparent 22px)',
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '32px', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', maxWidth: '240px' }}>
                            <span style={{ color: '#080808', fontSize: '1.5rem', fontWeight: '900', letterSpacing: '-1px', fontFamily: 'monospace' }}>
                                TANISHQ
                            </span>
                            <p style={{ color: 'rgba(8,8,8,0.5)', fontSize: '0.72rem', lineHeight: 1.8, fontFamily: 'monospace' }}>
                                Frontend developer from New Delhi. Building clean, fast and modern websites for brands that want to stand out.
                            </p>
                            <div style={{ display: 'flex', gap: '8px', marginTop: '4px' }}>
                                {[
                                    { href: 'https://github.com/098tanishq-source', Icon: FaGithub },
                                    { href: 'https://instagram.com/_friendly_dev',  Icon: FaInstagram },
                                    { href: 'mailto:098tanishq@gmail.com',           Icon: MdOutlineEmail },
                                ].map(({ href, Icon }) => (
                                    <a key={href} href={href} target="_blank" rel="noopener noreferrer"
                                        style={{ width: '34px', height: '34px', borderRadius: '8px', border: '1px solid rgba(8,8,8,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s', textDecoration: 'none' }}
                                        onMouseEnter={e => { e.currentTarget.style.background = 'rgba(8,8,8,0.08)'; e.currentTarget.style.borderColor = 'rgba(8,8,8,0.4)'; }}
                                        onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.borderColor = 'rgba(8,8,8,0.15)'; }}
                                    >
                                        <Icon style={{ color: 'rgba(8,8,8,0.6)', fontSize: '15px' }} />
                                    </a>
                                ))}
                            </div>
                        </div>
                        ...
                        <div style={{ display: 'flex', gap: '40px', flexWrap: 'wrap' }}>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                                <span style={{ color: 'rgba(8,8,8,0.3)', fontSize: '0.52rem', letterSpacing: '3px', textTransform: 'uppercase', fontFamily: 'monospace', marginBottom: '4px' }}>Work</span>
                                {[
                                    ...
                                    <a key={l.label} href={l.href} target="_blank" rel="noopener noreferrer"
                                        style={{ color: 'rgba(8,8,8,0.55)', fontSize: '0.72rem', textDecoration: 'none', fontFamily: 'monospace', transition: 'color 0.2s' }}
                                        onMouseEnter={e => e.currentTarget.style.color = '#080808'}
                                        onMouseLeave={e => e.currentTarget.style.color = 'rgba(8,8,8,0.55)'}
                                    >{l.label}</a>
                                ))}
                            </div>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                                <span style={{ color: 'rgba(8,8,8,0.3)', fontSize: '0.52rem', letterSpacing: '3px', textTransform: 'uppercase', fontFamily: 'monospace', marginBottom: '4px' }}>Contact</span>
                                {[
                                    { label: 'Email me',     href: 'mailto:098tanishq@gmail.com' },
                                    { label: 'Instagram DM', href: 'https://ig.me/m/_friendly_dev' },
                                    { label: 'GitHub',       href: 'https://github.com/098tanishq-source' },
                                ].map(l => (
                                    <a key={l.label} href={l.href} target="_blank" rel="noopener noreferrer"
                                        style={{ color: 'rgba(8,8,8,0.55)', fontSize: '0.72rem', textDecoration: 'none', fontFamily: 'monospace', transition: 'color 0.2s' }}
                                        onMouseEnter={e => e.currentTarget.style.color = '#080808'}
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '14px', alignItems: 'flex-end' }}>
                            <span style={{ color: 'rgba(8,8,8,0.3)', fontSize: '0.52rem', letterSpacing: '3px', textTransform: 'uppercase', fontFamily: 'monospace' }}>Status</span>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                <div style={{ width: '7px', height: '7px', borderRadius: '50%', background: '#16a34a', animation: 'pulse-dot 2s ease-in-out infinite' }} />
                                <span style={{ color: '#16a34a', fontSize: '0.65rem', fontFamily: 'monospace', letterSpacing: '1px' }}>Open to work</span>
                            </div>
                            <a href="mailto:098tanishq@gmail.com"
                                style={{ padding: '10px 20px', background: '#080808', border: '1px solid #080808', color: '#f0ece4', fontFamily: 'monospace', fontSize: '0.6rem', letterSpacing: '2px', textDecoration: 'none', borderRadius: '6px', textTransform: 'uppercase', transition: 'all 0.2s' }}
                                onMouseEnter={e => { e.currentTarget.style.background = '#333'; e.currentTarget.style.borderColor = '#333'; }}
                                onMouseLeave={e => { e.currentTarget.style.background = '#080808'; e.currentTarget.style.borderColor = '#080808'; }}
                            >
                                Hire Me
                            </a>
                        </div>
                    ...
                    {/* bottom bar */}
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px', marginTop: '32px', paddingTop: '20px', borderTop: '1px solid rgba(8,8,8,0.1)' }}>
                        <span style={{ color: 'rgba(8,8,8,0.35)', fontSize: '0.62rem', fontFamily: 'monospace' }}>
                            © {currentYear} Tanishq. All rights reserved.
                        </span>
                        <span style={{ color: 'rgba(8,8,8,0.35)', fontSize: '0.62rem', fontFamily: 'monospace' }}>
